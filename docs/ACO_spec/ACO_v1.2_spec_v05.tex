...
\clearpage
\section{Informative Note on Iterator-Bearing Strategies and Validation Boundaries}
\label{sec:iterator-bearing-strategies}

This section records an implementation-facing extension direction that has now been prototyped in the ACO processor and should be reflected in future reconciled revisions of the specification and user guidance.
It is informative and forward-looking.
It does not alter the normative core parsing and indentation rules stated above except where later revisions may choose to adopt the syntax described here as part of the stable surface language.

\subsection{Iterator-bearing Strategy syntax}

A particularly important near-term ACO extension is the ability to represent iterator-bearing Strategy nodes corresponding to iterator-bearing strategy forms already meaningful in APL.
A current compact surface syntax direction is:
\begin{quote}\ttfamily
...
This direction is motivated by the need to express in ACO a significant subset of the iterator-bearing strategy forms already present in the APL pattern corpus.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Instantiation-level operational realizability}: the current configured provider layer must actually be able to enumerate or compute the values denoted by the iterand against the available models, tools, or derived artefacts.
\end{enumerate}

The ACO processor need not itself execute iterator designators.
However, it must preserve enough syntactic structure to translate them into an APL form whose meaning is well-defined for the current O-ETB configuration.

\subsection{Term-valued iterands}

A key current design decision is that the \texttt{<iterand>} field may remain semantically opaque to the ACO processor, but must still be syntactically definite enough to be passed through deterministically as a term-like designator.
...
In the longer term, this points toward an extensible provider architecture in which the assurance engineer can tailor available iterator primitives to the models, tools, and derived artefacts available in a given project environment.

\subsection{Module declarations versus module occurrences}

A second important clarification now settled in the implementation design is the distinction between top-level module declarations and module occurrences within a local argument subtree.
A top-level \texttt{Module:} unit header introduces a reusable module definition.
By contrast, a local \texttt{Module [ID] label:} occurrence within a Goal/Strategy subtree is intended to denote a pure module/pattern reference leaf.
Children indented under such a module occurrence are therefore not intended to be part of the local supporting subtree and should be treated as structurally illegal in the stricter interpretation of the notation.

\subsection{Status}

The implementation has now been extended experimentally to recognize iterator-bearing Strategy headers and translate them into corresponding APL-facing representations while preserving backward compatibility against an automated regression suite.
Further specification cleanup, examples, and reconciliation with the GSN-oriented descriptive text remain as follow-on work.
