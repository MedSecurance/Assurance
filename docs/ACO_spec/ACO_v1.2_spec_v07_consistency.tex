...
\textcolor{blue}{In v1.2 revision 07, a further distinction is made between a top-level \texttt{Module:} unit header, which introduces a reusable module definition, and an in-tree \texttt{ModuleRef} occurrence, which invokes such a module within a local argument subtree. The specialised leaf form \texttt{GoalRef} is also recognised for local shared-goal reuse. These additions do not displace the core GSN-inspired node family, but they refine how module invocation and local structural aliasing are expressed in author-facing ACO.}

\paragraph{Evidence, ModuleRef, and GoalRef Nodes} are \emph{leaf-level} elements of an assurance case, though different kinds of leaves.
In particular:
\begin{itemize}[leftmargin=2em]
  \item an Evidence, ModuleRef, or GoalRef occurrence may appear as a child of a Goal or Strategy Node,
  \item an Evidence or ModuleRef occurrence may contain body text describing the artefact, its provenance,
        or how it supports the associated claim; a GoalRef occurrence is a compact alias form and normally has no body text, but
  \item no Goal, Strategy, Context, Assumption, Justification, Evidence, ModuleRef, or GoalRef node
        may appear beneath an Evidence, ModuleRef, or GoalRef occurrence in the outline,
  \item Context, Assumption or Justifications related to evidence or module references should instead be attached to the parent of the Evidence, ModuleRef, or GoalRef occurrence.
\end{itemize}
...
In accordance with established GSN practice, a ModuleRef occurrence in ACO is a leaf node with respect to the enclosing argument. Decomposition and supporting structure occur within the referenced module unit, not under the module reference itself.
Modular arguments allow a complex argument to be decomposed into independent, reusable argument fragments.

A GoalRef occurrence is also a leaf-level form. It does not introduce a new supporting subtree; rather, it denotes local reuse of an already established goal subtree within the same unit.

An Evidence Node says, ``justification ends here,'' while a ModuleRef Node says, ``justification continues (or is completed) elsewhere,'' and a GoalRef Node says, ``the same supporting argument already established elsewhere in this unit is being intentionally reused here.''
Tools should treat any attempt to introduce children under an Evidence, ModuleRef, or GoalRef occurrence as a
...
\textcolor{blue}{(v1.2 clarification) Evidence nodes are intended to be leaf-level dischargers of a goal: they do not introduce further decomposition structure beneath themselves in the ACO indentation tree. Tools may warn (or treat as an error in strict modes) if an Evidence, ModuleRef, or GoalRef occurrence has children.}
...
\textcolor{blue}{Two specialised reference forms are additionally recognised in revision 07:}
\begin{itemize}[leftmargin=2em]
  \item \textcolor{blue}{\texttt{ModuleRef [ID] [<callExpr>]:} --- an in-tree module invocation. The bracketed call expression denotes the referenced module or pattern together with any actual arguments. A short explanatory body may follow in the ordinary way after the colon.}
  \item \textcolor{blue}{\texttt{GoalRef <TargetId>} --- a compact local shared-goal reference. This is a leaf form, normally written without a trailing colon or body text. It denotes reuse of a goal subtree already established elsewhere in the same unit.}
\end{itemize}
...
\begin{verbatim}
    ...
    ModuleRef M1 [componentBehaviour({component})]:
...
\textcolor{blue}{In revision 07, an in-tree reference to a module is introduced using the node header syntax with the type \texttt{ModuleRef}. The earlier local occurrence form using \texttt{Module} is still accepted for backward compatibility but is retained only as a compatibility alias. New outlines should use \texttt{ModuleRef}.}

The general form of a module reference header is therefore:
\begin{quote}
\texttt{ModuleRef [ID] [<callExpr>]:}
\end{quote}

\textcolor{blue}{where \texttt{<callExpr>} is typically a module or pattern name with actual arguments, for example \texttt{[n\_foundational\_plane\_node(\{S\},\{N\})]}. The previous compatibility form \texttt{Module [ID] [<callExpr>]:} remains acceptable to the current processor but is no longer the preferred author-facing syntax.}
...
A ModuleRef occurrence participates in the indentation-defined structure in exactly the same way as other node types. By default:
\begin{itemize}[leftmargin=2em]
  \item a ModuleRef occurrence appearing as a child of a Goal contributes a
        \texttt{supported\_by} relation, and
  \item the referenced Module unit may itself have supporting children (e.g.\ sub-goals, strategies, or evidence),
...
using a \texttt{Module:} unit header or a local \texttt{ModuleRef} occurrence as appropriate; there is no implicit or anonymous module construct.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item it does not constrain the presence or absence of Module units or ModuleRef occurrences, and
...
The occurrence of a \texttt{Module:} header is not itself a module-reference node, but rather metadata similar to a Case header.
%

\subsection{ModuleRef and GoalRef Nodes: Syntax and Semantics}
\label{sec:module-semantics}

ModuleRef nodes provide a way to invoke structured argument fragments within an
ACO outline. They are particularly useful for reuse, decomposition by component or subsystem,
and for managing large arguments. GoalRef provides the corresponding compact in-unit notation for reusing a goal subtree that has already been established locally.

\paragraph{Syntax Recap.}
A ModuleRef node is introduced using the reference header syntax:
\begin{quote}
\texttt{ModuleRef [ID] [<callExpr>]:}
\end{quote}
The syntactic rules for \texttt{[ID]} are the same as for other node types. The \texttt{<callExpr>} field is a term-like call designator rather than a free short label and is intended to lower deterministically into the corresponding APL pattern-reference form.

\paragraph{Structural Semantics.}
Semantically, a ModuleRef occurrence behaves as a specialised form of Goal support:
\begin{itemize}[leftmargin=2em]
  \item a ModuleRef occurrence denotes a referenced module whose root claim is intended to be supported internally,
  \item its supporting structure is provided within the referenced module unit rather than as local indentation children, and
  \item when a ModuleRef occurrence appears as a child of another node, it contributes a
        \texttt{supported\_by} relation.
\end{itemize}

Thus, from the perspective of the Node Model, a ModuleRef occurrence participates in the same
local support relations as a Goal-like child while remaining a leaf in the enclosing unit.

\paragraph{Internal Structure.}
A ModuleRef occurrence must not have local structural children. The referenced Module unit may of course contain children of any node type permitted by the general rules,
including:
\begin{itemize}[leftmargin=2em]
...
This allows a Module unit to encapsulate a complete argument fragment.

\paragraph{Use and Interpretation.}
ModuleRef and GoalRef are optional: an ACO outline need not use them at all. When they are used,
...
\textcolor{blue}{GoalRef has an additional local semantic constraint: the referenced goal identifier must denote a goal already established within the same unit. The current processor resolves GoalRef using that unit-local symbol table discipline. A future semantic check will further confirm that the assumptions and contexts required by the reused subtree are satisfied at each reuse site.}

ACO v1.2 does not require the author to write internal ETB terms such as \texttt{goal\_ref/1} or \texttt{ac\_pattern\_ref/5}. Those are downstream forms handled by translation to APL and instantiation in O-ETB.

\paragraph{Tool Considerations.}
Tools should:
\begin{itemize}[leftmargin=2em]
  \item treat ModuleRef occurrences as first-class reference nodes in the argument graph,
  \item include them in diagnostics and statistics,
  \item allow ModuleRef occurrences to appear wherever a Goal could appear structurally, and
  \item avoid imposing additional constraints not stated in this specification.
\end{itemize}

This deliberately lightweight treatment ensures that ModuleRef and GoalRef can be used
...
\begin{itemize}[leftmargin=2em]
  ...
  \begin{itemize}[leftmargin=1.5em]
    ...
    \item \textcolor{blue}{a module reference whose call expression or actual parameter bindings are not yet provided,}
...
\subsection{Bodies of Goal and ModuleRef node headers}
...
\begin{itemize}[leftmargin=2em]
  \item Ambiguous wording in Goal/ModuleRef bodies (use of ``may'', ``etc.'', etc.).
...
\begin{itemize}[leftmargin=2em]
  \item total number of nodes and numbers by type (goals, strategies, contexts, assumptions, justifications, evidences, module references, and GoalRef occurrences),
...
Goal G0 operationalPlane:
  ...
  Goal G2 compositionBehaviour:
    ...
    ModuleRef M2 [compositionBehaviour({composition})]:
      Behaviour of the {composition} meets the local policy.

  ModuleRef M0 [interface()]:
...
(M1, module\_ref, componentBehaviour({component}))
(M2, module\_ref, compositionBehaviour({composition}))
(M0, module\_ref, interface())
\end{verbatim}
...
block(g2, goal,    compositionBehaviour,
      "Compositional behaviour of compositions ensures the local policy
       is met.").
ac_pattern_ref(m1, componentBehaviour, componentBehaviour, [{component}]).
ac_pattern_ref(m2, compositionBehaviour, compositionBehaviour, [{composition}]).
ac_pattern_ref(m0, interface, interface, []).
...
digraph OperationalPlane {
  ...
  M1 [label="ModuleRef M1: componentBehaviour({component})"];
  M2 [label="ModuleRef M2: compositionBehaviour({composition})"];
  M0 [label="ModuleRef M0: interface()"];
...
\clearpage
\section{Iterator-Bearing Strategies and Validation Boundaries}
\label{sec:iterator-bearing-strategies}

This section describes an extension that has now been implemented in the ACO processor to further exploit O-ETB APL capabilities.

\subsection{Iterator-bearing Strategy syntax}

A recent ACO extension is the ability to represent iterator-bearing Strategy nodes corresponding to iterator-bearing strategy forms already meaningful in APL.
The syntax is:
\begin{quote}\ttfamily
Strategy [ID] <shortLabel> for each <Var>:<Category> in <Iterand>:
\end{quote}
...
\begin{itemize}[leftmargin=2em]
  \item the Strategy short label is the part preceding the reserved phrase \texttt{for each},
  \item the iterator variable is \texttt{<Var>},
  \item the iterator category or role is \texttt{<Category>}, and
  \item the iterand designator is the term-valued field \texttt{<Iterand>}.
\end{itemize}

This direction is motivated by the need to express in ACO the iterator-bearing strategy forms already present in the APL pattern corpus.

\subsection{Validation boundaries}

Iterator-bearing syntax exposes an important architectural distinction among three different notions of well-formedness:
\begin{enumerate}[leftmargin=2em]
  \item \textbf{ACO-level syntactic well-formedness}: the iterator-bearing header must be definite enough to be parsed and translated deterministically.
  \item \textbf{APL-level semantic well-formedness}: the resulting iterand term must be recognized as a known and properly formed APL iterator/designator under the current instantiation/provider-layer configuration.
  \item \textbf{Instantiation-level operational realizability}: the instantiation layer must actually be able to enumerate or compute the values denoted by the iterand against the available models, tools, or derived artefacts.
\end{enumerate}

The ACO processor need not itself execute iterator designators.
However, it must preserve enough syntactic structure to translate them into an APL form whose meaning is well-defined for the current O-ETB implementation.

The implementation has been extended to recognize iterator-bearing Strategy headers and translate them into corresponding APL representations while preserving backward compatibility against an automated regression suite.

\subsection{Term-valued iterands}

The \texttt{<iterand>} field may remain semantically opaque to the ACO processor, but must still be syntactically definite enough to be passed through deterministically as a term-like designator.
Thus, the processor may validate shape without performing execution.
This allows ACO to remain readable while still covering a wider portion of the useful APL space.

\subsection{Validation API and provider architecture direction}

To support that boundary, a useful architectural direction is for O-ETB to expose an API that allows the ACO layer to ask whether a parsed iterand term is a known and properly formed APL iterator/designator under the current instantiation/provider-layer configuration.
Actual execution or enumeration remains a downstream concern.
In the longer term, we envision an extensible provider architecture in which the assurance engineer can tailor available iterator primitives to the models, tools, and derived artefacts available in a given project environment.

\section{Module declarations, ModuleRef, and GoalRef}

An important clarification now settled is the distinction between top-level module declarations and local reference occurrences within an argument subtree.

\subsection{Module declarations}

A top-level \texttt{Module:} unit header introduces a reusable module definition. It is metadata that begins a new unit, analogous in role to a \texttt{Case:} header. The body of that unit then contains the argument structure belonging to the module.

\subsection{ModuleRef occurrences}

By contrast, a local \texttt{ModuleRef} occurrence within a Goal/Strategy subtree denotes a pure module or pattern reference leaf. It is not itself the beginning of a new local supporting subtree. Children indented beneath such an occurrence are therefore structurally illegal in the stricter interpretation of the notation.

The preferred local syntax is now:
\begin{quote}
\texttt{ModuleRef [ID] [<callExpr>]:}
\end{quote}
...
For example:
\begin{quote}
\texttt{ModuleRef M0 [n\_foundational\_plane\_node(\{S\},\{N\})]:}
\end{quote}

The earlier local occurrence form using \texttt{Module} is still accepted by the processor for backward compatibility, but new outlines should use \texttt{ModuleRef}.

\subsection{GoalRef occurrences}

A second local reference form is now recognised:
\begin{quote}
\texttt{GoalRef <TargetId>}
\end{quote}

This is a compact leaf form for local shared-goal reuse. It denotes that the author intends to rely on the same already-established goal subtree elsewhere in the same unit, rather than restating or reinstantiating that subtree.

For example, if \texttt{Goal G4} has already been established within the enclosing unit, the author may later write:
\begin{quote}
\texttt{GoalRef G4}
\end{quote}

The local reference is intentionally author-facing and should not require the author to write the internal ETB term \texttt{goal\_ref/1}.

\subsection{Processing semantics}

The corresponding processing behaviour is now part of the reference implementation:
\begin{itemize}[leftmargin=2em]
  \item \texttt{ModuleRef} lowers to the existing APL pattern-reference mechanism.
  \item \texttt{GoalRef <TargetId>} lowers to the internal APL form \texttt{goal\_ref('<TargetId>')}.
  \item \texttt{GoalRef} resolution is currently unit-local and is checked against the local symbol table discipline of the enclosing case or module unit.
  \item A validity-check hook for shared-goal reuse has been inserted into the downstream execution path. In the current implementation this hook is present but conservative: it succeeds unconditionally, preserving the architectural seam for later refinement.
\end{itemize}

\subsection{Practical consequence for authors and tools}

The effect of these additions is to align the ACO surface language more closely with the settled processing model:
\begin{itemize}[leftmargin=2em]
  \item \texttt{Module:} defines reusable module units.
  \item \texttt{ModuleRef} invokes those units within a local argument subtree.
  \item \texttt{GoalRef} reuses an already-established local goal subtree within the same unit.
\end{itemize}

These forms improve clarity for both authors and reviewers by separating definition, invocation, and local structural reuse into distinct author-facing notations.
