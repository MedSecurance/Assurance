...
%\newcommand{\pipeline}{\ensuremath{\rightarrow}}
...
\begin{center}
{\LARGE \textbf{\textcolor{blue}{Assurance Case Outline (ACO) Specification v1.2}}}\\[0.5em]
\today
\end{center}
...
A prototype implementation for ACO is part of the Open Evidential Tool Bus (O-ETB) developed for the EC MedSecurance Project.

This v1.2 revision is a conservative evolution of v1.1. It preserves the ACO surface language previously defined with modest refinements, while clarifying and extending the behaviour of the accompanying front-end tooling (canonicalisation, diagnostics, ASCII tree view), and formalising author guidance and assessment concepts that are already used operationally in the O-ETB workflow.
...
This specification defines a structured textual outline language (``Assurance Case Outline'' -- ACO) for authoring assurance case instances and modules. It extends the interface presented the O-ETB toward the user, providing a gentle entry point to its use that does not involve the technical syntax of its Argument Pattern Language.

ACO is:
\begin{itemize}[leftmargin=2em]
  \item Intended for use by assurance engineers and assurance case authors in conjunction with O-ETB.
  \item Domain-independent and platform-neutral.
...
APL and diagrams are generated from it by the ACO Processor.
...
canonicalises, and analyses ACO outlines. This tool is referred to as the
\emph{ACO Frontend}. The ACO Frontend is a \emph{reference implementation} of this
...
tooling.
In the remainder of this document, the terms \emph{ACO Frontend} and, where the
context is clear, simply \emph{Frontend}, are used interchangeably to refer to
this reference implementation.

In addition, the Frontend:
\begin{itemize}[leftmargin=2em]
  \item parses ACO into the Node Model,
  \item computes hierarchical identifiers that reflect the indentation structure,
  \item can canonicalise an ACO outline (systematically renumbering IDs consistently), and
  \item can quickly generate several ASCII tree views for easier inspection, comprehension and review.
\end{itemize}

\textcolor{blue}{Version 1.2 treats input assessment and author guidance as first-class concerns: the front-end is expected to compute and report a set of quality signals (e.g.\ undeveloped nodes, provisional evidence, uninstantiated nodes) that can be used both for expert review and for ``wizard'' guidance for inexperienced authors.}
...
Nodes are grouped into Cases and Modules.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Evidence} --- an artefact or reference that directly discharges the goal it
        supports.
  \item \textbf{Module} --- a reusable or composite block that encapsulates a goal and the
        structure that supports it, such as:
...
\paragraph{Evidence Nodes as Leaves.}
Evidence nodes are \emph{leaf-level} elements of an assurance case. An Evidence node
does not introduce further decomposition structure beneath itself in the ACO indentation tree and therefore
\textbf{must not have child nodes} in the indentation structure.

In particular:
\begin{itemize}[leftmargin=2em]
  \item an Evidence node may appear as a child of a Goal, Strategy, or Module,
  \item an Evidence node may contain body text describing the artefact, its provenance,
  ...
  \item no Goal, Strategy, Context, Assumption, Justification, Evidence, or Module node
        may appear beneath an Evidence node in the outline,
  \item Context, Assumption or Justifications related to evidence should instead be attached to the parent of the Evidence node.
\end{itemize}

Tools should treat any attempt to introduce children under an Evidence node as a
...
Additional specialised constructs (e.g.\ richer module structures, structural abstractions or domain-specific node flavours) can be introduced later, but the initial ACO v1.x family focuses on these core types.

\textcolor{blue}{(v1.2 clarification) Evidence nodes are intended to be leaf-level dischargers of a goal: they do not introduce further decomposition structure beneath themselves in the ACO indentation tree. Tools may warn (or treat as an error in strict modes) if an Evidence node has children.}
...
planning, and review. Evidence nodes with provisional evidence categories are not considered to be fully developed.
...
This design keeps the surface language stable while allowing tools and workflows
...
\begin{itemize}[leftmargin=2em]
  \item provisional and accepted evidence are structurally equivalent,
  \item both may appear as children of Goals, Strategies, or Modules, and
...
When provisional evidence is detected, tools should:
\begin{itemize}[leftmargin=2em]
...
\begin{itemize}[leftmargin=2em]
  ...
  \item If the second token matches the pattern of an ID (e.g.\ \texttt{G\textbackslash d+}, \texttt{C\textbackslash d+}, \texttt{M\textbackslash d+}, etc.), it is treated as the \textbf{ID}.
...
\subsection{Optional Case Header}

In larger and more developed outlines it is often useful to name the overall assurance case explicitly.
ACO therefore supports an optional \emph{case header} at the top of the file, written
...
\emph{Module} headers. A Module encapsulates a goal together with the structure that supports it,
...
When a case header is absent, tools may generate a default or derived case identifier
for presentation purposes (e.g.\ in reports or visualisations), but this must not be
...
\begin{itemize}[leftmargin=2em]
  ...
  \item The IDs used must appear elsewhere in the assurance case as node IDs in headers of the permissible types.
\end{itemize}
...
Although authors are free to choose IDs, it is often convenient to follow a dotted, outline-style convention that mirrors the indentation tree, e.g.\ \texttt{G1.}, \texttt{G1.1}, \texttt{G1.1.2}. The reference ACO front-end can compute such hierarchical IDs automatically from the indentation structure and compare them with author-supplied IDs, issuing diagnostic messages if an explicit dotted ID does not match the structure implied by indentation.
...
The reference front-end also computes simple statistics such as:
\begin{itemize}[leftmargin=2em]
...
The ACO notation itself is tooling-neutral, but it is designed to be systematically mapped to an APL-style representation used by tooling such as O-ETB.
...
The precise surface syntax of APL is determined by the specification and implementation of O-ETB, but the mapping must be:
\begin{itemize}[leftmargin=2em]
...
Using a Prolog-like term notation for illustration (not a normative APL definition), the example in Section~7.1 could be represented conceptually as:
...
\textcolor{blue}{(v1.2 note) When cross-branch relations are present (via explicit NL relations), a DOT exporter may choose to draw them using a distinct style (e.g.\ dotted or coloured edges), or may emit a separate report listing cross-branch relations, to avoid obscuring the primary indentation-derived tree.}
...
surface language for authoring assurance case outlines. Nevertheless, a number
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Direct ACO to DOT rendering.} \\
        ...
        use and quick inspection, and would complement (not replace) the DOT
  ...
  \item \textbf{ACO Workbench.} \\
        ...
        diagnostics, statistics, ASCII tree views, and visualisation into a
        single interactive environment. This could include IDE plugins,
        standalone graphical tools, or web-based front-ends built on top of the
        reference ACO Frontend.
  ...
  \item \textbf{Enhanced mapping to downstream representations.} \\
        ...
        formal or semi-formal representations, including parameterisation and
        reuse mechanisms.

  \item \textbf{Additional analyses and metrics.} \\
        Support for more advanced structural or quality analyses over ACO
        outlines, beyond the lightweight diagnostics defined in this
        specification.
\end{itemize}

These extensions are deliberately deferred to preserve the simplicity and
stability of ACO v1.2. Their inclusion here signals likely evolution paths
...
Version 1.1 additionally documented:
\begin{itemize}[leftmargin=2em]
...
\section{Reference Front-End and Tooling Context}
\label{sec:frontend}

This specification is accompanied by a reference front-end implementation of ACO.
The ACO Frontend is a reference implementation of this specification and is used in
conjunction with the Open Evidential Tool Bus (O-ETB) in the MedSecurance Project.

The purpose of the reference front-end is to demonstrate the intended interpretation
...
only possible implementation, nor to impose constraints beyond those defined by this
specification.
...
The tool:
\begin{itemize}[leftmargin=2em]
  \item reads an ACO file (including an optional case header or pattern header),
...
In addition to raw Evidence counts, the front-end may classify Evidence nodes into
...
Typical provisional categories include (but are not limited to):
\begin{itemize}[leftmargin=2em]
  \item \textbf{Planned evidence} -- evidence that is identified but not yet available,
  \item \textbf{Placeholder evidence} -- a temporary stand-in used during early development,
...
\textcolor{blue}{This section presents assessment-related concepts and features associated with ACO
as a \emph{design study}. The material in this section describes intended and
exploratory capabilities and is not yet implemented in the current ACO Frontend.}

T\textcolor{blue}{he purpose of this discussion is to outline how assessment information, evidence
maturity, and argument status could be interpreted and exploited by future tools
built on top of ACO and O-ETB. The concepts described here do not affect the core
syntax or semantics of ACO v1.2 and should not be assumed to reflect existing
front-end behaviour unless explicitly stated.}

\subsection{\textcolor{blue}{Purpose}}

\textcolor{blue}{ACO is not only a surface authoring language: it is also the entry point for iterative improvement. Authors (and reviewers) require fast feedback about gaps and risks. To that end, the reference front-end should compute assessment signals that are:}
\begin{itemize}[leftmargin=2em]
  ...
  \item \textcolor{blue}{usable both in expert mode and in wizard-style author guidance.}
\end{itemize}

\subsection{\textcolor{blue}{Canonical Terminology: Node States}}

\textcolor{blue}{ACO v1.2 distinguishes the following node states, which must be tracked separately:}
...
\textcolor{blue}{A node may be uninstantiated only, undeveloped only, or both. Tools must report these categories distinctly rather than collapsing them into a single ``missing'' status.}

\subsection{\textcolor{blue}{Evidence Semantics and Provisional Categories}}
...
\begin{itemize}[leftmargin=2em]
  \item \textcolor{blue}{If the evidence category named by an evidence node is not defined in \texttt{KB/EVIDENCE/categories.pl}, but the knowledge base contains a reserved provisional entry (e.g.\ \texttt{evidence\_category(provisional,\_Desc,\_Type,\_ValidationMethod)}), then the category is treated as \emph{provisional}.}
...
\subsection{\textcolor{blue}{Assessment Dimensions (Vector-Valued)}}
...
\subsection{\textcolor{blue}{Confidence as a Derived Measure}}
...
\subsection{\textcolor{blue}{Evolutionary / Search-Based Improvement Model}}
...
\subsection{\textcolor{blue}{Author Guidance Modes}}
...
aco \pipeline [ACO-WORKBENCH] \pipeline aco \pipeline [ACO-to-APL] \pipeline apl \pipeline [INSTANTIATE]
    \pipeline repository(CASES,EVIDENCE) \pipeline [EXPORT_AC] \pipeline cap(txt, html, svg)
\end{verbatim}
...
\subsection{\textcolor{blue}{Role of AI Assistance (Out of Scope for v1.2)}}

\textcolor{blue}{AI-assisted transformation proposals and learned guidance policies are explicitly out of scope for v1.2. However, v1.2 defines the assessment vocabulary that would govern any future AI assistance. In particular, AI suggestions must remain advisory and should be evaluated using the same assessment metrics as algorithmic hints.}
