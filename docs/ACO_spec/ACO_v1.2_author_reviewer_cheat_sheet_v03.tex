...
\hspace*{1.5em}The operational plane guarantees the \{local policy\} is met.}

\Sec{Unit Headers}
\begin{itemize}
  \item \textbf{Case unit header:} (optional)\\
  \Example{Case: <Title> - <Scope>}          
  \item \textbf{Module unit header:}\\
  \Example{Module: <Name> [ (<formal parameter list>) ]}
\end{itemize}
\vspace{-12pt}
...
\hspace*{1.5em}Behaviour of the \{component\} satisfies the local policy.}
...
\begin{itemize}
  \item Referenced using standard header syntax: \texttt{Module [ID] label:}
...
\hspace*{1.5em}Behaviour of the \{component\} meets its local policy.}

\Sec{Syntax Notes}
\begin{itemize}
  \item \textbf{Explicit node relations (optional):} natural-language relation sentences (see page 2).
  \item \textbf{Placeholders:} use braces \texttt{\{...\}} and define them nearby or in a Context.
  \item \textbf{Units:} each \texttt{Case:} or \texttt{Module:} starts a unit; node IDs are unit-scoped.
\end{itemize}
...
\textbf{Checklist:}\hspace{0.6em}[ ] Top goals supported \hspace{0.6em}\hspace{0.6em}[ ] Evidence closes claims \hspace{0.6em}\hspace{0.6em}[ ] Assumptions justified \hspace{0.6em}\hspace{0.6em}[ ] No structural errors
}}\\[6pt]
...
\begin{itemize}
  \item \textbf{Supported-by:}\\
  \Example{G0 is supported by G1, G2 and M0.}
  \item \textbf{In-context-of:} \\
  \Example{C0 provides context for G0.}
  \item Explicit relations augment (do not replace or contradict) indentation-derived relations.
\end{itemize}
