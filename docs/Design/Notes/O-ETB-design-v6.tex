...
\section{Resets}\label{sec:resets}
A reset command is executed as follows:
\begin{itemize}
    \item The O-ETB command $reset.$ is first recognized by the command module which passes it on as $etb:etb\_reset/0$ defined in the $etb$ module.
    \item $etb:etb\_reset$ calls: $$evidence:detach\_evidence\_repository$$ and $$assurance:detach\_assurance\_repository$$ to assure that the in-memory persistent data is flushed.
    \item $etb:etb\_reset$ does a complete reset simply by invoking $$make~\text{-}s~clean$$ using the $makefile$ definition of the $clean$ target to remove and initialize files in both: $$REPOSITORY/CASES$$ and $$REPOSITORY/EVIDENCE$$ directories (currently no action is taken on $REPOSITORY/OUTLINES$).
    \item After the $make~clean$ operation the evidence repository is reattached with $$attach\_evidence\_repository$$ since there is only one evidence repository that should always be accessible to O-ETB.
\end{itemize}
There are also entry points in module $etb$ for $etb\_reset/1$ with the argument taking the possible values: $all$, $cap$, $repos$, $cases$, and $evidence$.
The different values for the argument have the following effects:
\begin{itemize}
    \item $all$ - causes $etb\_reset/0$ to be invoked.
    \item $cap$ - causes $export:reset\_CAP/0$ to be invoked.
    \item $repos$ - causes the invocation of: $$etb\_reset(cases)$$
    and $$etb\_reset(evidence)$$ to be invoked.
    \item $cases$ - causes $assurance:reset\_assurance\_repository$ to be invoked.
    \item $evidence$ - causes $evidence:reset\_evidence\_repository$ to be invoked.
\end{itemize}
The predicate $assurance:reset\_assurance\_repository$ uses "make" with the target $clean\_cases$ \emph{after} calling: $$assurance:detach\_assurance\_repository$$

The predicate $evidence:reset\_evidence\_repository$ uses "make" with the target $clean\_evidence$ \emph{after} calling: $$evidence:detach\_evidence\_repository$$
To restore access to the evidence repository it also invokes: $$evidence:attach\_evidence\_repository$$ \emph{after} calling $make$.

The predicate $export:reset\_CAP$  uses $make$ with the target $clean\_cap$.

The use of $make$ from within O-ETB process, while creating an external dependency, does guarantee consistency of the reset whether it is performed in O-ETB or outside the application as a $make$ command by having the $makefile$ provide the sole definition of the various $clean$ operations.
...
\subsection{Packaging the Kelly 6-Step Assistant as an Agent Skill}
...
\subsubsection{Layer 1: Method Layer --- Kelly 6-Step Assistant (Agent Skill)}
...
\subsubsection{Layer 2: Transformation Layer --- ACO Workbench}
...
\subsubsection{Layer 3: Execution and Evidence Layer --- O-ETB Core}
...
This separation supports the Principle of Least Astonishment\footnote{Daniel J. Edwards - A human-computer interaction should cause as little astonishment as possible to the human.}: authors reason about ACO artifacts; transformations preserve intent; execution layers do not introduce hidden semantic
...
\section{ACO Transformations (T1--T7) Formal Definition}\label{sec:formal_def}
...
\section{Iterator Validation Boundaries and Extensible Iterand Semantics}
\label{sec:iterator-validation-boundaries}

This section records an important architectural distinction for iterator-bearing constructs in ACO and APL, so that subsequent implementation work does not collapse several different concerns into a single mechanism.

\subsection{Motivation}

ACO is being extended toward fuller practical coverage of the APL constructs already used in O-ETB patterns and assurance case generation.
A particularly important class of constructs is the family of iterator-bearing strategy forms used to express repeated argument structure over a collection of entities, flows, hazards, modes, transitions, and similar designators.

Examples already present in the existing O-ETB/APL ecosystem include iterator designators such as:
\begin{quote}\ttfamily
nodes(S),\quad subjects(Policy),\quad processes(Policy),\quad ss\_flows(Policy),\\
ipc\_flows(Policy),\quad hazards(Policy),\quad modes(Subject),\quad transitions(Subject),\\
list(Values),\quad expand(X)
\end{quote}

These designators raise a subtle issue of architectural responsibility.
The ACO processor must be able to parse and translate an iterator-bearing construct reliably, but it need not itself \emph{execute} the iterator against a model, tool output, or knowledge base.

\subsection{Three distinct notions of well-formedness}

The design should explicitly distinguish the following three levels:

\begin{enumerate}
  \item \textbf{ACO-level syntactic well-formedness} \\
  At this level, the concern is whether the iterator-bearing ACO syntax is sufficiently definite to be parsed and translated deterministically.
  In particular, the \texttt{<iterand>} field must be syntactically well-formed enough to be recognized as a term-valued designator rather than arbitrary prose.

  \item \textbf{APL-level semantic well-formedness} \\
  At this level, the concern is whether the parsed iterand term denotes a known and properly formed APL iterator/designator.
  This is stronger than mere syntactic well-formedness, but weaker than actual execution.
  For example, \texttt{hazards(Policy)} may be recognized as a valid iterator designator form even if the current environment does not yet know how to enumerate the hazards of the given policy.

  \item \textbf{Instantiation-level operational realizability} \\
  At this level, the concern is whether the iterator can actually be evaluated in the current environment, using the available models, tools, knowledge bases, or cached analysis artefacts.
  This is where actual expansion occurs.
\end{enumerate}

These three levels should not be conflated.

\subsection{Design consequence for ACO}

The current design decision is that the \texttt{<iterand>} field in an iterator-bearing ACO construct may be \emph{semantically opaque} to the ACO processor, but it must still be \emph{syntactically definite} enough to translate reliably.

Accordingly:

\begin{itemize}
  \item the ACO processor does \emph{not} need to execute iterator designators;
  \item the ACO processor \emph{does} need to parse them into a term-like representation deterministically;
  \item the resulting ACO\,$\rightarrow$\,APL translation must be semantically well-defined;
  \item execution or expansion of the iterator remains a downstream concern.
\end{itemize}

This means that a term-valued representation of \texttt{<iterand>} is acceptable and desirable.

\subsection{Architectural implication: an O-ETB validation API}

To support the distinction between ACO-level parsing and downstream execution, O-ETB should expose an API that allows the ACO layer to ask whether a parsed iterand term is a known and properly formed APL iterator/designator.

Conceptually, the interface would support a question of the form:

\begin{quote}\ttfamily
is\_this\_a\_known\_and\_well\_formed\_APL\_iterator\_designator(Term)?
\end{quote}

or, more explicitly,

\begin{quote}\ttfamily
apl\_iterand\_well\_formed(+Term,\,-Diagnostic)
\end{quote}

The purpose of such an API is \emph{validation}, not execution.
In particular, it would allow the ACO processor to reject or warn about malformed iterator designators without taking on responsibility for enumerating their values.

\subsection{Extensible provider architecture}

A more general issue also arises.
Many useful iterands do not refer merely to fixed built-in collections.
Rather, they refer to:
\begin{itemize}
  \item special interfaces into \texttt{KB/MODELS},
  \item outputs of analysis tools or model transformations,
  \item derived artefacts such as hazards, modes, transitions, or flows, or
  \item other project-specific sources of information about the system under scrutiny.
\end{itemize}

This suggests that iterator execution should eventually be mediated by an extensible provider architecture rather than being hard-wired monolithically inside a single instantiation module.

The intended division of responsibilities is therefore:

\begin{itemize}
  \item \textbf{ACO layer:} parse iterator-bearing syntax and normalize the iterand into a term;
  \item \textbf{APL/O-ETB validation layer:} determine whether the term is a known and properly formed iterator/designator;
  \item \textbf{Instantiation/provider layer:} evaluate the iterator against the relevant model, tool, repository, or derived artefact source.
\end{itemize}

This architecture allows the assurance engineer, rather than necessarily the assurance case author, to configure and extend the available iterator primitives to suit the artefacts and tools available for a given project.
...
\begin{itemize}
  \item ACO should remain readable and author-oriented; it should not collapse into raw APL notation.
  \item Iterator-bearing syntax should lower deterministically into an existing APL representation.
  \item Validation should distinguish clearly among:
  \begin{itemize}
    \item syntactically well-formed iterators,
    \item semantically well-formed APL iterator designators, and
    \item operationally available iterator providers.
  \end{itemize}
  \item The eventual iterator/provider mechanism should be extensible and configurable by the assurance engineer.
\end{itemize}

\subsection{Status}

This section records a design decision and architectural direction.
It does not imply that the current implementation already provides the full validation API or provider architecture described above.
Those remain explicit follow-on design and implementation tasks.

\section{ModuleRef, GoalRef, and Compact Actual-Binding Rendering}
\label{sec:moduleref-goalref-compact-bindings}

This section records the completed implementation of the settled ACO surface
forms for module invocation and local shared-goal reuse, together with the
corresponding improvements to textual export.

\subsection{Author-facing ACO forms}

The ACO language now distinguishes clearly between reusable module
\emph{definitions} and in-tree module \emph{invocations}.

\begin{itemize}
\item \texttt{Module:} is reserved for unit headers that define reusable modules.
\item \texttt{ModuleRef} is the in-tree author-facing form for invoking a module.
\item \texttt{GoalRef} is the in-tree author-facing form for local shared-goal reuse.
\end{itemize}

Thus, an author writes a module definition as a \texttt{Module:} unit, but
writes a module call within a case or module body using \texttt{ModuleRef}.
Likewise, local reuse of an already established goal subtree is expressed as
\texttt{GoalRef G4} rather than by exposing the internal ETB term
\texttt{goal\_ref/1} directly in the ACO.

\subsection{Lowering to APL and execution semantics}

The ACO front end and ACO-to-APL translator were updated accordingly.

\begin{itemize}
\item \texttt{ModuleRef} lowers to the existing ETB pattern-reference mechanism.
\item \texttt{GoalRef G4} lowers to the internal APL form
      \texttt{goal\_ref('G4')}.
\end{itemize}

The semantics of \texttt{GoalRef} are those previously settled for local shared
goals:

\begin{enumerate}
\item The first encountered occurrence defines the concrete instantiated goal
      subtree.
\item Later occurrences are non-instantiating aliases of that same subtree.
\item Each alias induces the ordinary structural support relation from the
      shared subtree root to the parent in which the alias appears.
\end{enumerate}

In the current implementation, resolution of \texttt{GoalRef} is unit-local.
This matches the present design scope and local symbol-table discipline.

A hook for the eventual shared-goal validity condition was inserted into the
execution path for \texttt{goal\_ref/1}. Its current implementation is a stub
that succeeds unconditionally. This preserves the correct architectural seam
for later refinement, in which reuse validity will be checked against the
assumptions and contexts required by the shared subtree.

\subsection{Foundational Plane validation}

The revised Foundational Plane ACO replaced the earlier helper-module
workaround with the intended local-reuse design.

Specifically:

\begin{itemize}
\item the helper module used only to simulate local reuse of the OS
      certification subtree was removed;
\item the node-level module now defines the \texttt{osCertification} subtree
      once; and
\item runtime reconfiguration reuses that subtree via \texttt{GoalRef G4}.
\end{itemize}

The resulting instantiated and exported case demonstrates the intended
behavior:

\begin{itemize}
\item each instantiated node module contains exactly one concrete
      \texttt{osCertification} subtree;
\item runtime reconfiguration reuses that subtree locally within the same node
      module; and
\item no extra helper-module artifact is created.
\end{itemize}

This aligns the new ACO/APL path with the earlier built-in foundational
baseline on the load-bearing local-sharing semantics.

\subsection{Compact rendering of module actual bindings in TXT export}

The textual exporter was also improved to reduce the large raw actual-term
dumps previously shown under successful module invocations.

For successful defined module references, the preferred rendering now preserves
the formal placeholder shape on the left of the rewrite arrow and shows compact
actual bindings inline on the right. For example:

\begin{quote}
\texttt{module M0\_1\_k2 : n\_foundational\_plane\_node([S,N]) \\
\hspace*{2em}-> n\_foundational\_plane\_node\_G10\_3([S=\#S1, N=\#N2])}
\end{quote}

This form has several advantages.

\begin{itemize}
\item It preserves the author-visible formal interface
      \texttt{n\_foundational\_plane\_node([S,N])}.
\item It presents the instantiation as a simple and visually symmetric rewrite
      from formal placeholders to compact bound values.
\item It avoids overwhelming the main assurance-case text with large concrete
      argument structures.
\end{itemize}

Compact IDs are allocated per formal placeholder and bound actual value, so
the same actual bound repeatedly to the same formal is rendered by the same
compact ID within the export. Thus, if the same platform value is bound to
\texttt{S} in multiple calls, it may appear throughout the case as
\texttt{\#S1}; distinct node values bound to \texttt{N} may appear as
\texttt{\#N1}, \texttt{\#N2}, and so forth.

\subsection{Delimited actuals legend}

To preserve inspectability and support downstream processing, the full actual
values corresponding to compact IDs are emitted once, after the complete TXT
assurance case, in a sharply delimited legend section:

\begin{quote}
\texttt{===== END ASSURANCE CASE =====}\\
\texttt{===== BEGIN ACTUALS LEGEND =====}\\
\texttt{\#S1 = platform(main,\ldots)}\\
\texttt{\#N1 = node(node1,\ldots)}\\
\texttt{\#N2 = node(node2,\ldots)}\\
\texttt{===== END ACTUALS LEGEND =====}
\end{quote}

This arrangement keeps the main case body readable while giving tools and human
readers a single crisp demarcation between the assurance case proper and the
binding legend.

The legend currently appears only in TXT export. A future refinement will
provide a corresponding HTML presentation, most likely via a side panel or
expandable details view, while keeping SVG output visually clean.

\subsection{Status}

The implementation of \texttt{ModuleRef}, \texttt{GoalRef}, the local
shared-goal execution path, and compact TXT actual-binding rendering is now in
place. The combined foundational and operational pattern-list instantiation
path has been rerun successfully, and the ETB self-test suite is green.
