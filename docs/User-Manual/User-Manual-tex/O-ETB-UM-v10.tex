...
% Common Unicode seen in the manual
...
\newcommand{\dtitle}{%
  Open Evidential Tool Bus (O-ETB) v1.1.3\\User Manual\\December 2025
}
%\newcommand{\dshorttitle}{O-ETB v1.1.3 User Manual}
\newcommand{\dshorttitle}{User Manual}
\newcommand{\dversion}{8}  % Document Version number
...
\paragraph{Unit-scoped identifiers.}
...
\paragraph{Instantiation and scope lifting.}
...
\paragraph{Pattern instantiation occurrence.}
...
\paragraph{Construction of instantiated identifiers.}
...
\paragraph{Properties of the scheme.}
...
\paragraph{Relationship to the instantiation workflow.}
...
\end{forest}
\caption*{\footnotesize IDs (e.g., \texttt{G10}, \texttt{E10}) are unique only within their unit.}
\end{minipage}
\hfill
\begin{minipage}[t]{0.485\textwidth}
\centering
...
\end{forest}
\caption*{\footnotesize One instance identifier (\texttt{17}) is allocated for this instantiation occurrence; all nodes from that occurrence share the suffix.}
\end{minipage}

\caption{Worked example: unit-scoped ACO identifiers lifted into case-wide scope during pattern instantiation.}
