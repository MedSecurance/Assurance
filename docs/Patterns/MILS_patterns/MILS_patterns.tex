...
\newcommand{\dnumber}{0}
\newcommand{\dtitle}{%
  Construction of Assurance Case\\Argument Pattern Catalogues for O-ETB\\(with MILS Case Study)
}
\newcommand{\dshorttitle}{Constructing O-ETB Argument Patterns}
\newcommand{\dversion}{0.2}  % Document Version number
...
The MILS Argument Patterns and descriptions contained in this document were developed by atsec information systems and other partners of the CITADEL Project.
Groundwork for MILS assurance cases was laid by partners of the prior EC D-MILS Project (Distributed MILS, Project Number 318772), in particular, University of York
\cite{D-MILS-WWW,D-MILS-D2.3,D-MILS-D2.4,D-MILS-D4.2,D-MILS-D4.3}.
Copyright in portions of this document borrowed from the cited references remains vested in the respective Project Partners.
...
new realization of the concept for Adaptive MILS, first appearing as AM-ETB and now through continuing development by The Open Group as embodied in the O-ETB (Open Evidential Tool Bus).
...
\subsection{Assurance Case Evaluation Criteria}
\label{sec:eval}

There are three aspects to the criteria for evaluation of an assurance case.

\begin{itemize}
\item Generic, objective, quantitative, criteria for every assurance case.
\item Qualitative criteria about content of a specific assurance case
\item Qualitative and quantitative criteria about the efficiency of the production of a specific assurance case
\end{itemize}
Some of the quantitative criteria were formalised in \cite{D-MILS-D2.4}.

\subsubsection{Metrics and Sanity Checks}

Some simple metrics and sanity checks can be performed for an assurance case, particularly one which is at least partly instantiated automatically from a detailed system model and appropriate assurance case argument patterns. These allow the assurance case developer to be confident that all the appropriate elements are there and have been discussed. Although such metrics alone cannot measure the {\it quality} of the argument or evidence, they can:

\begin{itemize}
\item Check that all nodes in the final argument are instantiated and developed. If they are not they can be highlighted and a justification provided if necessary (for example, a placeholder for functionality which will be integrated later).
\item Highlight criticality of certain nodes in terms of the argument structure. For example, how many claims are supported by a single solution (evidence item). If many claims are supported by a single item of evidence, then changing that evidence will have a high impact on the argument. Highlighting this issue allows us to consider whether the evidence can be decomposed further in order to ameliorate the potential impact of a change.
\item Ensure that all relevant elements of an input model, or input data, are included. For example, do all components in an architecture model have a corresponding argument module. As another example, have all hazards or threats have been mentioned in a claim somewhere in the argument.
\end{itemize}

\subsubsection{Content Assessment}

The purpose of the assurance case is to provide a compelling case to show that the system is fit for purpose. Further, it may need to show that several processes running on the same platform are fit for purpose and sufficiently protected from one another.  As such, it needs to demonstrate the the configured system is sufficiently dependable, safe and secure for the related and identified reliability hazards and threats. For example, will a particular data asset be secure from a particular kind of unauthorized modification and will a particular hazard be mitigated by effective detection and recovery. The assurance case should also provide confidence that there is no undesired emergent behavior.

Assessment of such aspects may need to be determined on a case by case basis. Whilst a process and checklist for review can be defined, the content of different assurance cases will differ so much that the results may be difficult to compare. Users and evaluators should be provided with a checklist of highlighted aspects to examine closely, and in turn they are expected to provide independent feedback on the usefulness of the assurance case.

\subsubsection{Efficiency of the assurance case approach}

In addition to providing a compelling assurance case, we want to construct an assurance case in an efficient and maintainable way. A non-modular, manually constructed assurance case may be compelling, but with laborious and error-prone production methods, especially in the ongoing maintenance of the assurance case in the face of system evolution, the is cost of the benefits may be too high for some application domains. The cost of producing the assurance case initially, re-usability of argument parts, and efficient maintainability of the argument (e.g. when a component is changed) must be considered. 

It is expected that the model based approach with substantial automated instantiation of assurance case patterns to build an assurance case will be less costly than manual methods.
At present, there may be no known assurance cases nor argument patterns for certain domains, manually constructed or otherwise. Thus there is no current baseline against which to easily measure improvements in cost and time in new domains. However, we can, roughly compare with the manual construction of assurance cases in other domains (e.g. safety domain). Once an initial baseline has been developed, we can show improvements in the time taken to incorporate changes, e.g. to regenerate an assurance case following system changes such as component alteration or replacement. 

The re-usability of modular and argument pattern-based assurance case development can be demonstrated and assessed for supporting platform arguments by showing they can be re-used for multiple product certification projects that use the same platform. For example, are the assurance argument patterns for the platform applicable without substantial alteration to a different application project that uses the same platform. The re-usability of the patterns can be assessed by instantiating them for numerous components and observing how robust they are to variations, e.g. in component type, scale and error types. This activity has the side benefit of providing an opportunity to improve the parameterization and versatility of the argument pattern catalog.

The maintainability aspect, can be evaluated by generating typical change scenarios, and assessing the impact that has on the argument. For example, how many claims in a module (and related modules) are affected by the introduction of a new error or software requirement, or other alteration. Another test might be a modified or alternative architectural configuration of the same components. Finally, it would be beneficial to evaluate the argument pattern catalog's ability to create or maintain an application assurance case in the face of changes to the platform or to the operational environment. 

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\newpage
\section{Case Study: MILS Domain Assurance Case}\label{sec:ac-structure}
...
\item Adaptive MILS framework: since all planes in the Adaptive MILS framework are largely static, patterns for the various planes are established to create arguments specific to each of the planes to simplify instantiation and evaluation of the assurance case:
	...
	\item Certification assurance plane: create an argument about ETB tool integration and the verification and validation of results
	\end{itemize}
\item Interface: Create an argument that the communication between components or compositions in the architecture only occurs via connections explicitly defined in the system model
\item Threats: Create an argument that threats are sufficiently mitigated
\item States and transitions: Create arguments that states and state transitions of components are in accordance with the states and transitions as specified in the system model
\item Modes and transitions: Create arguments that modes and mode transitions of compositions are in accordance with the modes and transitions as specified in the system model
\item Composition: Create arguments that formally defined properties of a system, as specified in the modeling language architecture description, are implemented and satisfied by a system.
...
Furthermore, the O-ETB should support the addition of supplementary (custom) assurance patterns, which are required to conform to the specification of the assurance case argument patterns (see Section~\ref{sec:ap-rules}). In addition, it is important to note that the argument patterns are used as aid for the instantiation of the assurance case, and may be modified and are further defined upon instantiation. 
...
\includegraphics[height=0.8\textheight,keepaspectratio]{figures/patterns/foundationalplane3.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.8,width=.8\textwidth,height=0.8\textheight]{figures/patterns/adaptationplane4-1.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.4,width=\textwidth,height=0.8\textheight,keepaspectratio]{figures/patterns/adaptationplane4-2.pdf}
...
The certification assurance plane comprises the O-ETB, which enables tool integration, as well as the verification and validation of results. The O-ETB encompasses different techniques and mechanisms to generate and manage the evidence needed for certification during system operation. The O-ETB provides a framework for propagating system-level and inter-component constraints, enforcing standards, checking consistency, and providing automated review of evidence as a proxy for certification experts (Figure ~\ref{fig:certificationassurance-plane}). 
...
Assurance case argument patterns have the ability to reduce the subjectivity of assurance cases, but only if the system models are correctly specified. Therefore, it is required that the local policy, or security requirements (if applicable) are clearly represented in the description of the system model.  

The O-ETB tool automatically identifies parts of argument patterns that are to be instantiated. The system model differentiates between systems, processes and compositions to facilitate the automated instantiation of the assurance case argument patterns (Figure ~\ref{fig:automaticinstantiation}).
...
Which patterns are to be instantiated are automatically identified on basis of the system model as shown in Figure~\ref{fig:automaticinstantiation}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=.8\textwidth]{figures/automaticinstantiation.png}
\caption{\label{fig:automaticinstantiation} Identification of patterns to be instantiated}
\end{center}
\end{figure}

\subsubsection{Automated Instantiation Based on the System Model}\label{sec:instantiation-systemmodel}
As opposed to using both the argument patterns and system model to construct the assurance case, another manner of establishing the assurance case may be by solely using the system model. The proposed way of achieving this is to specify the safety and/or security requirements, assumptions and evidence requirements in the system model directly. These requirements may be derived from regulatory standards or specified by the system owner. The specification of the various components and subcomponents of the system in the system model are taken to establish the structure of the assurance case. The advantage of this approach is that the arguments in the assurance case are completely tailored to a particular system, and do not have to be further refined subsequent to construction to the same extent as when generating an assurance case from a generic set of patterns. It has to be emphasised, however, that the safety and/or security requirements need to be carefully considered and specified in the system model and that any potential threats are identified and included when establishing the requirements in order to prevent confirmation bias. Taking a standards-based approach may aid in assuring all relevant security and safety claims, and assumptions are addressed for a particular type of system. 
