...
regression_test :-
	...
	etb_regression_tests(Regression),
        aco_parser_regression_tests(ACO_Regression),
	append([Startup,Regression,ACO_Regression],AllTests),
	forall(member(T,AllTests), test:report_test(etb:T)).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Test cases, test passes if tc_xx goal succeeds

% ACO transform tests
tc_aco_t1_01 :- tc_aco_a2_aux(t1_slim_evidence_file, 'E111').

tc_aco_t2_01 :- tc_aco_a1_aux(t2_observe_file).

tc_aco_t2_02 :- tc_aco_a2_aux(t2_insert_goal_file, pair(goal('G10'),evidence('E101'))).

tc_aco_t6_01 :- tc_aco_a2_aux(t6_modularize_file, ['G1', 'G11', 'G51']).

tc_aco_t7_01 :- tc_aco_a1_aux(t7_observe_file).

tc_aco_t7_02 :- tc_aco_a2_aux(t7_insert_strategy_file, goal('G10')).

tc_aco_a2_aux(PName, Arg) :-
    tc_aco_files(PName, FullInFile, TmpFile, FullExpFile),
    Test =.. [PName, Arg, FullInFile, TmpFile],
    call(aco_transforms:Test),
    read_file_to_string(TmpFile, Got, [newline(detect)]),
    delete_file(TmpFile),
    read_file_to_string(FullExpFile, Exp, [newline(detect)]),
    !, Got == Exp.

tc_aco_a1_aux(PName) :-
    tc_aco_files(PName, FullInFile, FullExpFile),
    Test =.. [PName, FullInFile, Got],
    call(aco_transforms:Test),
	open(FullExpFile,read,S),
	read_term(S, Exp, []), close(S),
    !, Got == Exp.


% tc_aco_files/4 +PName, -FullInFile, -TmpFile, -FullExpFile
tc_aco_files(PName, FullInFile, TmpFile, FullExpFile) :-
    tc_aco_files(PName, '.aco', '.aco', FullInFile, FullExpFile),
    tmp_file_stream(text, TmpFile, S),
    close(S).
    % do in the caller, tho not necessary, b/c Prolog does on normal exit
    % delete_file(TmpFile)

% tc_aco_files/3 +PName, -FullInFile, -FullExpFile - no tmp file
tc_aco_files(PName, FullInFile, FullExpFile) :-
	tc_aco_files(PName, '.aco', '.cand', FullInFile, FullExpFile).
...
tc_aco_files(PName, InExt, OutExt, FullInFile, FullExpFile) :-
    ...
    atomic_list_concat([AcoExpDir, ExpFileName], '/', FullExpFile).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% experimental new test engine for ACO+APL -- WIP
